...
import { AppError } from '@sharedUtils/error.utils';
...
export class ServicioUsuario {
    ...
    getLibrosPendientes(id: number): Observable<LibroResumen[]> {
        return this.http.get<LibroResumen[]>(`${this.apiUrl}/usuario/libros/pendientes/${id}`).pipe(
            map((resp) => {
                ...
                const librosPendientes = procesarRespuestaArray<LibroResumen>(
                    resp,
                    'libros_pendientes',
                );
                if (!librosPendientes) {
    ...
    getEventosCreados(id: number): Observable<EventoResumen[]> {
        return this.http.get<EventoResumen[]>(`${this.apiUrl}/usuario/eventos/creados/${id}`).pipe(
            map((resp) => {
                ...
                const eventosCreados = procesarRespuestaArray<EventoResumen>(
                    resp,
                    'eventos_creados',
                );
                if (!eventosCreados) {
    ...
    getEventosAsistidos(id: number): Observable<EventoResumen[]> {
        return this.http
            .get<EventoResumen[]>(`${this.apiUrl}/usuario/eventos/asistidos/${id}`)
            .pipe(
                map((resp) => {
                    console.log('Respuesta del servidor para obtener eventos asistidos:', resp);
                    const eventosAsistidos = procesarRespuestaArray<EventoResumen>(
                        resp,
                        'eventos_asistidos',
                    );
                    if (!eventosAsistidos) {

...
import {
    procesarRespuestaArray,
    procesarRespuestaUnica,
} from '@app/shared/utils/procesarRespuesta';
...
export class ServicioDetalleEvento {
    ...
    private getComentariosPorIdEvento(id: number): Observable<EventoComentario[]> {
        ...
        return this.http.get<{ data: EventoComentario[] }>(url).pipe(
            map((resp) => {
                console.log(
                    '[ServicioDetalleEvento] getComentariosPorIdEvento - respuesta HTTP:',
                    resp,
                );
    ...
    private getAsistentesPorIdEvento(id: number): Observable<EventoUsuario[]> {
        ...
        return this.http.get<{ data: { asistentes: EventoUsuario[] } }>(url).pipe(
            map((resp) => {
                console.log(
                    '[ServicioDetalleEvento] getAsistentesPorIdEvento - respuesta HTTP:',
                    resp,
                );
                return procesarRespuestaArray<EventoUsuario>(
                    { data: resp.data.asistentes },
                    'asistentes',
                );
            }),
            catchError((error) => {
